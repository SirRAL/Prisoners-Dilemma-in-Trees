...
def test() -> None:
    ...
    game_tree = GameTree()
    game_tree.add_decision_branch(True)
    game_tree._c_branch.times_picked += 1

    game.decisions[game.curr_round] = (True, True)
    subtree = game_tree.find_subtree_by_moves((True, True))
    subtree.times_picked += 1
    game_tree._c_branch.update_pick_chance()

    game.curr_round += 1

    subtree.add_decision_branch(True)
    subtree._c_branch.times_picked += 1
    game.decisions[game.curr_round] = (True, False)
    subtree2 = subtree.find_subtree_by_moves((True, False))
    subtree2.times_picked += 1
    subtree._c_branch.update_pick_chance()

    game.curr_round += 1
    subtree2.avg_pts_gained = game.get_points(1)

    game_tree.update_avg_pts_gained(game)
    ...
    subtree.add_decision_branch(True)
    game.decisions[game.curr_round] = (True, True)
    subtree2 = subtree.find_subtree_by_moves((True, True))
    subtree2.times_picked += 1
    subtree._c_branch.update_pick_chance()
...
def test2():
    ...
    game.decisions[game.curr_round] = (True, True)
    game_tree.update_after_round(game)
    subtree = game_tree.find_subtree_by_moves((True, True))
    game.curr_round += 1

    # second round
    game.decisions[game.curr_round] = (True, False)
    subtree.update_after_round(game)
    subtree2 = subtree.find_subtree_by_moves((True, False))

    # post-game
    subtree2.avg_pts_gained = game.get_points(1)
    game_tree.update_after_game(game)
